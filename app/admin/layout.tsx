...
const NAV_KEYS = [
  ...
  { href: "/admin/chats", labelKey: "nav.chats", icon: "chats", group: "ktc" },
...
function AdminLayoutInner({ children }: { children: React.ReactNode }) {
  ...
  // 1:1 채팅 안읽음 (어드민 SELECT RLS 정책으로 클라이언트에서 직접 카운트)
  useEffect(() => {
    if (!authorized) return;
    ...
    return () => {
      clearInterval(interval);
      window.removeEventListener("chat-read", fetchChatUnread);
    };
  }, [authorized]);
